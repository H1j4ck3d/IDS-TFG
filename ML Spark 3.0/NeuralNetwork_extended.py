...
dataset = spark.read.csv("../Dataset/extended_dataset.csv", mode='DROPMALFORMED', schema=schema, header='true')
dataset.show(25)
print(dataset.count())
...
layers4 = [9, 9, 9, 10]
layers6 = [9, 18, 10, 6, 8, 10]

now = datetime.datetime.now()
print (now.year, now.month, now.day, now.hour, now.minute, now.second)

# Creamos el modelo de red neuronal, lo entrenamos y realizamos la prediccion
mpc = MultilayerPerceptronClassifier(maxIter=100, layers=layers4, labelCol='attack_cat_index', featuresCol='features', blockSize=128, seed=1234)
mpc = mpc.fit(train)
